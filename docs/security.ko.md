...
custom render handler는 신뢰하는 애플리케이션 코드입니다. 출력 대상에 직접 쓰며
내장 safe HTML 정책이 escape하지 않습니다. 신뢰하지 않는 extension을 활성화하지
말고, 각 HTML handler가 소스에서 온 값을 출력 문맥에 맞게 escape하도록 하세요.
...
inline delimiter planner는 모든 적대적 패턴에서 선형임이 아직 증명되지
않았습니다. fuzzing 뒤에도 입력 제한과 요청 deadline을 배포 정책의 일부로
사용하세요.
